...
class KUPSR(SequentialModel):
    ...
    def _define_params(self):
        ...
        # 定义融合层
        self.fusion_layer = nn.Linear(2 * self.emb_size, self.emb_size)
    ...
    def calculate_user_similarity(self, user_embeddings):
        """
        计算用户之间的余弦相似度
        :param user_embeddings: 用户的嵌入矩阵，形状为 (num_users, emb_size)
        :return: 用户之间的相似度矩阵，形状为 (num_users, num_users)
        """
        # 计算余弦相似度
        norm_user_embeddings = F.normalize(user_embeddings, p=2, dim=-1)
        similarity_matrix = torch.matmul(norm_user_embeddings, norm_user_embeddings.t())
        return similarity_matrix

    def get_similar_users_by_threshold(self, user_id, similarity_matrix, threshold=0.05):
        """
        获取与当前用户相似度大于指定阈值的用户
        :param user_id: 当前用户ID
        :param similarity_matrix: 用户相似度矩阵
        :param threshold: 相似度阈值
        :return: 与当前用户相似度大于阈值的用户ID列表
        """
        similarity_scores = similarity_matrix[user_id]
        # 筛选相似度大于阈值的用户
        mask = (similarity_scores > threshold)
        similar_users = torch.where(mask)[0]
        # 去除当前用户自身
    ...
    def forward(self, feed_dict):
        ...
        user_embeddings = self.user_embeddings.weight.data  # 所有用户的嵌入 (num_users, emb_size)

        # 计算用户之间的相似度
        similarity_matrix = self.calculate_user_similarity(user_embeddings)

        # 选择相似度在指定范围内的用户并进行注意力加权
        new_user_embeddings = []
        for user_id in user_ids:
                similar_users = self.get_similar_users_by_threshold(user_id, similarity_matrix, threshold=0.05)
                similar_user_embeddings = user_embeddings[similar_users]  # (k, emb_size)
                current_user_embedding = user_embeddings[user_id]  # (emb_size)

                # 使用注意力机制结合用户特征和相似用户项目的重要性
                project_importance = self.attention(current_user_embedding, similar_user_embeddings)  # (k)
                # 加权求和得到新的用户嵌入
                fused_embedding = torch.sum(similar_user_embeddings * project_importance.unsqueeze(1), dim=0)
                fused_embedding = torch.cat([current_user_embedding, fused_embedding], dim=0)  # (2 * emb_size)

                # 使用 Bi-LSTM 处理上下文向量
                output, (h_last, c_last) = self.blstm(
                    fused_embedding.unsqueeze(0).unsqueeze(0))  # 输入形状为 [1, 1, 2*emb_size]
                # 取 Bi-LSTM 的最后一个时间步的隐藏状态
                hidden = torch.cat([h_last[-2, :, :], h_last[-1, :, :]], dim=1)
                new_user_vector = self.fusion_layer(hidden)  # 进行特征融合
                new_user_embeddings.append(new_user_vector)

        # 更新用户嵌入
        new_user_embeddings = torch.stack(new_user_embeddings, dim=0)
        # 确保 new_user_embeddings 的形状为 [256, 64]，移除额外的维度
        new_user_embeddings = new_user_embeddings.squeeze(1)
        # 重新设置user_embeddings的weight.data
        self.user_embeddings.weight.data[user_ids.long()] = new_user_embeddings
    ...
    def attention(self, user_embedding, similar_embeddings):
        """
        使用注意力机制计算项目的重要性权重
        :param user_embedding: 当前用户嵌入向量 (emb_size)
        :param similar_embeddings: 相似用户的嵌入向量 (k, emb_size)
        :return: 项目的重要性权重 (k)
        """
        # 计算注意力分数
        attention_scores = torch.matmul(similar_embeddings, user_embedding)
